...
模型 key 也存在这台机器上。这些东西搬不进云端沙箱，所以干活的那一端必须是你自己
的机器。问题在于：
...
手机浏览器（外网）通过 HTTPS/WSS 连到公网 VPS 的 Nginx（443 端口），Nginx 反代
到中继 service/main.py（127.0.0.1:9000）。中继按 device_token 把消息单转给家里 PC
的桥 pc/opencode_bridge.py（通过出站 WebSocket）。桥翻译成 opencode HTTP/SSE 协议，
opencode 真正干活（LLM、工具、文件）。

四类角色、四段链路：

| 角色 | 跑在哪 | 职责 |
|---|---|---|
| 手机浏览器 | 外网 | 手机控制台（page.html，由中继直接提供） |
...
哑管道（dumb pipe）：形容这个中继"哑"在哪——对转发的内容不解析、不缓存、
不产生：
...
两个方向都不 parse JSON：转发语义就是"这台设备的屏幕，所有持 token 的手机都看
...
| ask | 转发 permission_request 给手机；PERM_TIMEOUT（默认 120s）无应答自动拒绝 |
...
| proxy_buffering off | Nginx 攒满 buffer 才吐给浏览器，手机端"转圈半天、回复一次性蹦出来" |
...
另由中继 RELAY_WS_MAX 管）；中继用 systemd 常驻（Restart=always + NoNewPrivileges/PrivateTmp）。
