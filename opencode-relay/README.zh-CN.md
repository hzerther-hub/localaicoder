...
手机打开 `https://你的域名/s/?d=<token>`，家里 PC 就在指尖：

![手机控制台演示](docs/screenshot-phone-console.png)

*手机控制台实拍：左侧选项目/会话，右侧聊天。图中这场会话上传了一张截图并提问识图，
opencode 一边作答、工具调用（`glob …`）一边流式刷进聊天流；底部是模型选择器与权限
模式。你看到的一切都是家里 PC 上的 opencode 干的——手机只负责发帧和渲染流。*
