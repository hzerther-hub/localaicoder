...
> 目标域名：**biancheng.mei.bz**。请**严格按本文件顺序**在服务器上执行；
> 每条命令可直接复制。任何一步出错先停，勿跳过安全项。

## 0. 背景（30 秒看懂）

- **客户端**：用户电脑上的 Local AI Studio（Go，出站 WS 连 `wss://biancheng.mei.bz/client?d=<token>`）
- **服务器**：`relay-server/`（FastAPI），只按 token 路由，不解析业务
- **手机**：浏览器打开 `https://biancheng.mei.bz/s/?d=<token>` 即得网页控制台
- **一个 token 两用**：桌面连服务器的凭证 + 手机链接的 `d=`。

## 1. 前置：DNS 与防火墙

1. 把 `biancheng.mei.bz` 解析到本服务器公网 IP（A 记录）。
2. 放行 80/443：
   ```bash
...
1. 用户打开 📱 面板 → 🌐 自建中继。
2. 服务器地址填：`wss://biancheng.mei.bz`（填 `https://biancheng.mei.bz` 也可，桌面端自动转 `wss://`）。
3. 点「生成」得到 64 位 token → 写入服务器 `config.json` 的 `device_tokens[]` → `systemctl restart relay-server`。
4. 用户点「连接」，状态点变绿即成功。
5. 手机打开：`https://biancheng.mei.bz/s/?d=<同token>`。
...
| 现象 | 排查 |
|---|---|
| 桌面「连接」后状态常红 | `curl -k https://biancheng.mei.bz/s/?d=<token>` 看是否 403；token 是否在服务器白名单；Caddy 是否已 reload |
| 手机打开 403 | 链接里 `d=<token>` 与白名单不一致 |
| 手机能开但没反应 | 桌面端是否已「连接」且 `Running`；设备未上线时服务器会拒绝新手机 |
| 域名连不通 | `dig biancheng.mei.bz` 是否解析到本机；80/443 是否放行 |
